...
//        this.screenFacade.warmCache()
    ...
    private void initClassLoader() {
        ...
        ZClassLoader.addCommonClass("org.moqui.entity.EntityValue", EntityValue.class);
        ZClassLoader.addCommonClass("EntityValue", EntityValue.class);
        ZClassLoader.addCommonClass("org.moqui.entity.EntityList", EntityList.class);
        ZClassLoader.addCommonClass("EntityList", EntityList.class);

        ClassLoader pcl = Thread.currentThread().getContextClassLoader()!= null?Thread.currentThread().getContextClassLoader():this.getClass().getClassLoader();
        if(pcl == null)pcl = System.class.getClassLoader();

        zClassLoader = new ZClassLoader(pcl);
...
//        if (emptyDbLoad == null || emptyDbLoad.equals("none") ) return false;
...
//                if (!emptyDbLoad.equals("all")) edl.dataTypes(new HashSet<>(Arrays.asList(emptyDbLoad.split(","))));
//
//                try {
//                    long startTime = System.currentTimeMillis();
...
//
//                    EntityDataLoader edl = ec.getEntity().makeDataLoader();
//                    edl.dataTypes(new HashSet<>(Arrays.asList("test")));
//
//                    try {
//                        long startTime = System.currentTimeMillis()；
//                        long records = edl.load()；
...
//    }
    ...
    @Nonnull
    @Override
    public String getZEntityVersion() {
        return null;
    }
...
//        if (logger.traceEnabled) logger.trace("Creating new ExecutionContext in thread [${currentThread.id}:${currentThread.name}]")
    ...
    public void destroyActiveExecutionContext() {
        ...
        if (ec != null) {
            ...
            this.activeContextMap.remove(Thread.currentThread().getId());
        }
    }
...
//    }
    ...
    @Override @Nonnull public L10nFacade getL10n() { return getEci().getL10n(); }
...
//    @Override @Nonnull ScreenFacade getScreen() { screenFacade }
    ...
    @Override @Nonnull
    public ClassLoader getClassLoader() { return this.zClassLoader; }
    @Override @Nonnull
    ...
    public synchronized Class compileGroovy(String script, String className) {
        ...
        if (groovyCompileCacheToDisk && hasClassName) {
            ...
            for (Object compiledClass : compiledClasses) {
...
//
//        ArrayList<String> sortedNames = [] as ArrayList<String>
