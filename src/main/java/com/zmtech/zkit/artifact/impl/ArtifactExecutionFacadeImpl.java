package com.zmtech.zkit.artifact.impl;

import com.zmtech.zkit.artifact.ArtifactExecutionFacade;
...
import com.zmtech.zkit.exception.ArtifactAuthorizationException;
...
public class ArtifactExecutionFacadeImpl implements ArtifactExecutionFacade {
    ...
    public ArtifactExecutionFacadeImpl(ExecutionContextImpl eci) {
        this.eci = eci;
    }

    public Map<String, Boolean> getScreenPermittedCache() {
        if (screenPermittedCache == null) screenPermittedCache = new HashMap<>();
        return screenPermittedCache;
    }

    @Override
    public ArtifactExecutionInfo peek() { return this.artifactExecutionInfoStack.peekFirst(); }

    @Override
    public ArtifactExecutionInfo push(String name, ArtifactExecutionInfo.ArtifactType typeEnum, ArtifactExecutionInfo.AuthzAction actionEnum, boolean requiresAuthz) {
    ...
    @Override
    public void push(ArtifactExecutionInfo aei, boolean requiresAuthz) {
        ArtifactExecutionInfoImpl aeii = (ArtifactExecutionInfoImpl) aei;
        pushInternal(aeii, requiresAuthz, true);
    }
    public void pushInternal(ArtifactExecutionInfoImpl aeii, boolean requiresAuthz, boolean countTarpit) {
        ...
        if (!isPermitted(aeii, lastAeii, requiresAuthz, countTarpit, true, null)) {
            ...
            for (ArtifactExecutionInfo warnAei : curStack) warning.append("\n").append(warnAei.toString());
    ...
    public String printHistory() {
        ...
        for (ArtifactExecutionInfo aei : artifactExecutionInfoHistory) aei.print(sw, 0, true);
        return sw.toString();
    }
    ...
    public void enableAuthz() { authzDisabled = false; }
    public boolean getAuthzDisabled() { return authzDisabled;}

    public boolean disableTarpit() { boolean alreadyDisabled = tarpitDisabled; tarpitDisabled = true; return alreadyDisabled ;}
    public void enableTarpit() { tarpitDisabled = false; }
    // boolean getTarpitDisabled() { return tarpitDisabled }

    public boolean disableEntityEca() { boolean alreadyDisabled = entityEcaDisabled; entityEcaDisabled = true; return alreadyDisabled; }
    public void enableEntityEca() { entityEcaDisabled = false; }
    public boolean entityEcaDisabled() { return entityEcaDisabled; }

    public boolean disableEntityAuditLog() { boolean alreadyDisabled = entityAuditLogDisabled; entityAuditLogDisabled = true; return alreadyDisabled; }
    public void enableEntityAuditLog() { entityAuditLogDisabled = false; }
    public boolean entityAuditLogDisabled() { return entityAuditLogDisabled; }

    public boolean disableEntityFkCreate() { boolean alreadyDisabled = entityFkCreateDisabled; entityFkCreateDisabled = true; return alreadyDisabled; }
    public void enableEntityFkCreate() { entityFkCreateDisabled = false; }
    public boolean entityFkCreateDisabled() { return entityFkCreateDisabled ;}

    public boolean disableEntityDataFeed() { boolean alreadyDisabled = entityDataFeedDisabled; entityDataFeedDisabled = true; return alreadyDisabled }
    public void enableEntityDataFeed() { entityDataFeedDisabled = false; }
    public boolean entityDataFeedDisabled() { return entityDataFeedDisabled; }
     ...
     * @param eci
     */
    public static boolean isPermitted(String resourceAccess, ExecutionContextImpl eci) {
    ...
    public boolean isPermitted(ArtifactExecutionInfoImpl aeii, ArtifactExecutionInfoImpl lastAeii, boolean requiresAuthz, boolean countTarpit,
        ...
        boolean isEntity = ArtifactExecutionInfo.AT_ENTITY == artifactTypeEnum;
        ...
        ArtifactExecutionInfoImpl.ArtifactAuthzCheck denyAacv = null;
        ArtifactExecutionInfoImpl.ArtifactAuthzCheck allowAacv = null;
        ...
        try {
            ...
            ArrayList<ArtifactExecutionInfoImpl.ArtifactAuthzCheck> aacvList = new ArrayList<>();
            ArrayList<ArtifactExecutionInfoImpl.ArtifactAuthzCheck> origAacvList = ufi.getArtifactAuthzCheckList();
            int origAacvListSize = origAacvList.size();
            for (int i = 0; i < origAacvListSize; i++) {
                ArtifactExecutionInfoImpl.ArtifactAuthzCheck aacv = origAacvList.get(i);
            ...
            for (int i = 0; i < aacvListSize; i++) {
                ...
                if (aacv.filterMap != null && aeii.parameters != null) {
                    ...
                    for (Map.Entry<String, Object> filterEntry : filterMapObj.entrySet()) {
    ...
    public static class AuthzFilterInfo {
    ...
    public ArrayList<AuthzFilterInfo> getFindFiltersForUser(String findEntityName) {
        EntityDefinition findEd = eci.entityFacade.getEntityDefinition(findEntityName);
        return getFindFiltersForUser(findEd, null);
    }
    public ArrayList<AuthzFilterInfo> getFindFiltersForUser(EntityDefinition findEd, Set<String> entityAliasUsedSet) {
        ...
        ArtifactExecutionInfoImpl lastAeii = (ArtifactExecutionInfoImpl) artifactExecutionInfoStack.peekFirst();
        ArtifactExecutionInfoImpl.ArtifactAuthzCheck aacv = lastAeii.internalAacv;
        ...
        ArrayList<AuthzFilterInfo> authzFilterInfoList =  null;
        for (int i = 0; i < authzFilterSize; i++) {
            ...
            for (int j = 0; j < entFilterSize; j++) {
                ...
                if (memberFieldAliases != null && entityAliasUsedSet != null) {
                    ...
                    Map<String, ArrayList<MNode>> newFieldAliases = null;

                    for (Map.Entry<String, ArrayList<MNode>> aliasesEntry : memberFieldAliases.entrySet()) {
                        ArrayList<MNode> aliasList = aliasesEntry.getValue();
                        if (aliasList == null) continue;// should never happen, buy yeah
                        ArrayList<MNode> newAliasList = null;

                        int aliasListSize = aliasList.size();
                        for (int ali = 0; ali < aliasListSize; ali++) {
                            MNode aliasNode = aliasList.get(ali);
    ...
    public ArrayList<EntityConditionImplBase> filterFindForUser(EntityDefinition findEd, Set<String> entityAliasUsedSet) {
        ...
        try {
            for (int i = 0; i < authzFilterInfoListSize; i++) {
                AuthzFilterInfo authzFilterInfo = authzFilterInfoList.get(i);
