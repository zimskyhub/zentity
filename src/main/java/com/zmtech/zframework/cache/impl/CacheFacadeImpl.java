...
import com.zmtech.zframework.tools.impl.ZCacheToolFactory;
...
public class CacheFacadeImpl implements CacheFacade {
    ...
    @Override
    public void clearAllCaches() { for (Cache cache : localCacheMap.values()) cache.clear(); }
    ...
    @Override
    public Cache getCache(String cacheName) { return getCacheInternal(cacheName, "local"); }
    @Override
    ...
    @Override
    public boolean cacheExists(String cacheName) { return localCacheMap.containsKey(cacheName); }
    @Override
    public Set<String> getCacheNames() { return localCacheMap.keySet(); }
    ...
    public List<Map<String, Object>> getAllCachesInfo(String orderByField, String filterRegexp) {
        ...
        if (orderByField != null && !orderByField.isEmpty()) CollectionUtil.orderMapList(ci, Arrays.asList(orderByField));
        return ci;
    }

    protected MNode getCacheNode(String cacheName) {
        MNode cacheListNode = ecfi.getConfXmlRoot().first("cache-list");
        MNode cacheElement = cacheListNode.first( (MNode it) -> { return it.getName().equals("cache")  && it.attribute("name").equals(cacheName);});
        ...
        if (cacheElement == null) cacheElement = cacheListNode
                .first((MNode it) ->{ return it.getName().equals("cache") && cacheName.startsWith(it.attribute("name"));});
        return cacheElement;
    }
    ...
    protected synchronized Cache initCache(String cacheName, String defaultCacheType) {
        ...
        if (cacheNode != null) {
            ...
            */
                ...
                logger.info("Initializing cache "+cacheName+" which has a CacheManager of type "+cacheManager.getClass().getName()+" and extended configuration not supported, using simple MutableConfigutation");
    ...
    public List<Map<String,Object>> makeElementInfoList(String cacheName, String orderByField) {
        ...
        if (cache instanceof ZCache) {
            ...
            for (Cache.Entry ce : mCache.getEntryList()) {
                ZEntry entry = (ZEntry)ce.unwrap(ZEntry.class);
            ...
            if (orderByField != null && !orderByField.isEmpty() ) CollectionUtil.orderMapList(elementInfoList, Arrays.asList(orderByField));
