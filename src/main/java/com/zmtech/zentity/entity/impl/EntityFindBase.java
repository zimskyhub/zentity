...
import com.zmtech.zentity.exception.EntitySqlException;
...
public abstract class EntityFindBase implements EntityFind {
    ...
    public final static int defaultResultSetType = ResultSet.TYPE_FORWARD_ONLY;
    ...
    public EntityFindBase(EntityFacadeImpl efi, String entityName) {
        this.efi = efi;
        this.entityName = entityName;
        TransactionFacadeImpl tfi = (TransactionFacadeImpl)efi.ecfi.getTransaction();
    ...
    public EntityFindBase(EntityFacadeImpl efi, EntityDefinition ed) {
        this.efi = efi;
    ...
    @Override
    public EntityFind entity(String name) { entityName = name; return this; }
    @Override
    ...
    @Override
    public EntityFind condition(String fieldName, EntityCondition.ComparisonOperator operator, Object value) {
        ...
        if (ed.isViewEntity && fi.fieldNode.attribute("function") != null) {
            return havingCondition(new FieldValueCondition(fi.conditionField, operator, value));
        } else {
            if (EntityCondition.EQUALS.equals(operator)) return condition(fieldName, value);
            return condition(new FieldValueCondition(fi.conditionField, operator, value));
        }
    }

    @Override
    public EntityFind condition(String fieldName, String operator, Object value) throws EntityException {
    ...
    @Override
    public EntityFind conditionToField(String fieldName, EntityCondition.ComparisonOperator operator, String toFieldName) {
    ...
    public EntityFind condition(Map<String, Object> fields) {
        ...
        if (fieldsSize == 1 && noSam && noScf) {
            ...
            singleCondField = onlyEntry.getKey();
            singleCondValue = onlyEntry.getValue();
        } else {
            if (noSam) simpleAndMap = new LinkedHashMap<>();
            if (!noScf) {
                simpleAndMap.put(singleCondField, singleCondValue);
    ...
    public EntityFind condition(EntityCondition condition) {
        ...
        if (condClass == FieldValueCondition.class) {
            ...
            if (EntityCondition.EQUALS.equals(fvc.getOperator()) && !fvc.getIgnoreCase()) {
            ...
            if (EntityCondition.AND.equals(basicCond.getOperator())) {
    ...
    @Override
    public EntityFind conditionDate(String fromFieldName, String thruFieldName, Timestamp compareStamp) {
    ...
    @Override
    public boolean getHasHavingCondition() { return havingEntityCondition != null; }
    ...
    @Override
    public EntityCondition getWhereEntityCondition() { return getWhereEntityConditionInternal(getEntityDef()); }
    public EntityConditionImplBase getWhereEntityConditionInternal(EntityDefinition localEd) {
        ...
        if (samSize > 0) {
            ...
            for (Map.Entry<String, Object> samEntry : simpleAndMap.entrySet()) {
                ConditionField cf;
                if (localEd != null) {
                    ...
                    cf = new ConditionField((String) samEntry.getKey());
                }
                condList.add(new FieldValueCondition(cf, EntityCondition.EQUALS, samEntry.getValue()));
            }
        }
        if (condList.size() > 0) {
            if (!wecNull) {
                ...
                if (whereEntCondClass == ListCondition.class) {
                    ...
                    if (EntityCondition.AND.equals(listCond.getOperator())) {
    ...
    public Map<String, Object> getSimpleMapPrimaryKeys() {
        int samSize = simpleAndMap != null ? simpleAndMap.size() : 0;
    ...
    @Override
    public EntityCondition getHavingEntityCondition() { return havingEntityCondition; }

//    @Override
//    public EntityFind searchFormInputs(String inputFieldsMapName, String defaultOrderBy, boolean alwaysPaginate) {
...
//            for (Map.Entry<String, Object> dpEntry : defaultParameters.entrySet()) ec.contextStack.put(dpEntry.getKey(), dpEntry.getValue());
//        }
//
//        // always look for an orderByField parameter too
//        String orderByString = inputFieldsMap != null && !inputFieldsMap.isEmpty()? (String) inputFieldsMap.get("orderByField"): defaultOrderBy;
...
//        if ((alwaysPaginate != null || inputFieldsMap?.get("pageIndex") || inputFieldsMap?.get("pageSize")) && !shouldCache()) {
...
//    }
    ...
    public EntityFind selectField(String fieldToSelect) {
        ...
        if (fieldToSelect.contains(",")) {
            for (String ftsPart : fieldToSelect.split(",")) {
    ...
    @Override
    public EntityFind selectFields(Collection<String> selectFields) {
        if (selectFields == null && selectFields.isEmpty()) return this;
        for (String fieldToSelect : selectFields) selectField(fieldToSelect);
        return this;
    }
    @Override
    public List<String> getSelectFields() { return fieldsToSelect; }
    ...
    public EntityFind orderBy(String orderByFieldName) {
        ...
        if (orderByFieldName.contains(",")) {
            for (String obsPart : orderByFieldName.split(",")) {
                String orderByName = obsPart.trim();
                FieldOrderOptions foo = new FieldOrderOptions(orderByName);
                if (getEntityDef().isField(foo.getFieldName()) && !this.orderByFields.contains(orderByName)) this.orderByFields.add(orderByName);
            }
        } else {
            FieldOrderOptions foo = new FieldOrderOptions(orderByFieldName);
            if (getEntityDef().isField(foo.getFieldName()) && !this.orderByFields.contains(orderByFieldName)) this.orderByFields.add(orderByFieldName);
        }
        return this;
    }
    @Override
    public EntityFind orderBy(List<String> orderByFieldNames) {
        if (orderByFieldNames == null orderByFieldNames.isEmpty()) return this;
        if (orderByFieldNames instanceof RandomAccess) {
            ...
            for (String orderByFieldName : orderByFieldNames) orderBy(orderByFieldName);
        }
        return this;
    }
    @Override
    public List<String> getOrderBy() { return orderByFields != null ? Collections.unmodifiableList(orderByFields) : null; }
    public ArrayList<String> getOrderByFields() { return orderByFields; }

    @Override
    public EntityFind useCache(Boolean useCache) { this.useCache = useCache; return this; }
    @Override
    ...
    @Override
    public EntityFind distinct(boolean distinct) { this.distinct = distinct; return this; }
    @Override
    public boolean getDistinct() { return distinct; }

    @Override
    public EntityFind offset(Integer offset) { this.offset = offset; return this; }
    @Override
    public EntityFind offset(int pageIndex, int pageSize) { return offset(pageIndex * pageSize);}
    @Override
    public Integer getOffset() { return offset; }

    @Override
    public EntityFind limit(Integer limit) { this.limit = limit; return this; }
    @Override
    public Integer getLimit() { return limit; }

    @Override
    public int getPageIndex() { return offset == null ? 0 : (offset/getPageSize()); }
    @Override
    public int getPageSize() { return limit != null ? limit : 20; }

    @Override
    public EntityFind forUpdate(boolean forUpdate) {
        this.forUpdate = forUpdate;
        this.resultSetType = forUpdate ? ResultSet.TYPE_SCROLL_SENSITIVE : defaultResultSetType;
        return this;
    }
    @Override
    public boolean getForUpdate() { return this.forUpdate; }

    // ======================== JDBC Options ==============================

    @Override
    public EntityFind resultSetType(int resultSetType) { this.resultSetType = resultSetType; return this; }
    @Override
    public int getResultSetType() { return this.resultSetType; }

    @Override
    public EntityFind resultSetConcurrency(int rsc) { resultSetConcurrency = rsc; return this; }
    @Override
    public int getResultSetConcurrency() { return this.resultSetConcurrency; }

    @Override
    public EntityFind fetchSize(Integer fetchSize) { this.fetchSize = fetchSize; return this; }
    @Override
    public Integer getFetchSize() { return this.fetchSize; }

    @Override
    public EntityFind maxRows(Integer maxRows) { this.maxRows = maxRows; return this; }
    @Override
    ...
    @Override
    public EntityFind disableAuthz() { disableAuthz = true; return this; }
//    @Override
//    public EntityFind requireSearchFormParameters(boolean req) { this.requireSearchFormParameters = req; return this }
    ...
    public boolean shouldCache() {
        ...
        if (useCache != null) {
            boolean useCacheLocal = useCache;
    ...
    public String toString() {
        return "Find: "+entityName+" WHERE ["+singleCondField+
                ":"+singleCondValue.toString()+"] ["+simpleAndMap
                +"] ["+whereEntityCondition+"] HAVING ["+havingEntityCondition+"] " +
...
//        }
    ...
    public abstract EntityDynamicView makeEntityDynamicView();
...
//            if (enableAuthz) aefi.enableAuthz()
    ...
    protected EntityValue oneInternal(EntityContextImpl ec, EntityDefinition ed) throws EntityException, SQLException {
        ...
        if (singleCondField != null && ed.isPkField(singleCondField) && ObjectUtil.isEmpty(singleCondValue)) {
        ...
        if (ftsSize == 0 || (txCache != null && txcValue == null) || (doCache && cacheHit == null)) {
            ...
            for (int i = 0; i < ftsSize; i++) {
                ...
                if (fi == null) {
                    ...
                    fi = ed.getFieldInfo(foo.getFieldName());
    ...
    public EntityConditionImplBase getConditionForQuery(EntityDefinition ed, EntityConditionImplBase whereCondition) {
    ...
    public abstract EntityValueBase oneExtended(EntityConditionImplBase whereCondition, FieldInfo[] fieldInfoArray,
        ...
        if (txcEli != null) {
            ...
            if (getDistinct() && fieldsToSelect != null && fieldsToSelect.size() > 0 && orderByExpandedSize > 0) {
                for (int i = 0; i < orderByExpandedSize; i++) {
                    ...
                    if (!fieldsToSelect.contains(foo.getFieldName())) fieldsToSelect.add(foo.getFieldName());
                }
            }
...
//            if (enableAuthz) ec.artifactExecutionFacade.enableAuthz()
    ...
    protected EntityListIterator iteratorInternal(EntityContextImpl ec, EntityDefinition ed) throws EntityException, SQLException {
        ...
        if (getDistinct() && fieldsToSelect != null && fieldsToSelect.size() > 0 && orderByExpanded.size() > 0) {
            for (String orderByField : orderByExpanded) {
                FieldOrderOptions foo = new FieldOrderOptions(orderByField);
                if (!fieldsToSelect.contains(foo.getFieldName())) fieldsToSelect.add(foo.getFieldName());
            }
        }
    ...
    public abstract EntityListIterator iteratorExtended(EntityConditionImplBase whereCondition, EntityConditionImplBase havingCondition,
...
//            if (enableAuthz) ec.artifactExecutionFacade.enableAuthz()
    ...
    protected long countInternal(EntityContextImpl ec, EntityDefinition ed) throws EntityException, SQLException {
        ...
        Cache<EntityCondition, Long> entityCountCache = doCache ? ed.getCacheCount(efi.getEntityCache()) : null;
        Long cacheCount = null;
        if (doCache) cacheCount = entityCountCache.get(whereCondition);
    ...
    public abstract long countExtended(EntityConditionImplBase whereCondition, EntityConditionImplBase havingCondition,
                                FieldInfo[] fieldInfoArray, FieldOrderOptions[] fieldOptionsArray) throws SQLException;

    @Override
    public long updateAll(Map<String, ?> fieldsToSet) {
    ...
    @Override
    public void extract(SimpleEtl etl) {
        ...
        try {
            ...
            logger.warn("EntityFind extract stopped on: " + (e.getCause() != null?e.getCause(): e.toString()));
        } finally {
            eli.close();
        }
    }
    ...
    @Override
    public ArrayList<String> getQueryTextList() { return queryTextList; }
}
